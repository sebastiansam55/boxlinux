...
class boxlinux():
	...
	def __init__(self):
		...
		parser.add_argument('-dl', metavar=takesfile, type=str, dest='dlFileName', help=_("Filename to download (Needs --dir if file in different dir)"))
		parser.add_argument('-dla', action='store_true',  help=_("Download all files"))
		parser.add_argument('-u', metavar=takesfile, type=str, dest='ulFileName', help=_("Filename to upload"))
		parser.add_argument('--setup', action='store_true', help=_("Setup for use"))
		parser.add_argument('-lsh', action='store_true', help=_("List filenames for human view"))
		parser.add_argument('-ls', action='store_true', help=_("Print filenames on Box to stdout"))
		parser.add_argument('--http-proxy', metavar=_('USERNAME:PASSWORD@PROXYIP:PORT'), type=str, dest='http_proxy', help=_("HTTP proxy info"))
		parser.add_argument('--https-proxy', metavar=_('USERNAME:PASSWORD@PROXYIP:PORT'), type=str, dest='https_proxy', help=_("HTTPS proxy info"))
		parser.add_argument('--getcomm', metavar=takesfile, type=str, dest='commentFileName', help=_("Fetch Comments for file"))
		parser.add_argument('--mkcomm', metavar=takesfile, type=str, dest='comment', help=_("Use with --dir as the filename"))
		parser.add_argument('--mkdir', metavar=_('FOLDERNAME'), type=str, dest='newFolder', help=_("Creates New folder on Box"))
		parser.add_argument('--rmdir', metavar=_('FOLDERNAME'), type=str, dest='delFolder', help=_("Deletes folder on Box"))
		parser.add_argument('--rm', metavar=takesfile, type=str, dest='delFile', help=_("Deleted file on Box"))
		parser.add_argument('-c', metavar=_('Comment'), type=str, dest='strComment', help=_("String to comment with"))
		parser.add_argument('--dir', metavar=_('DIR'), type=str, dest='ActionDir', help=_("Box directory to perform action in/on"))
		parser.add_argument('--sync', help=_("Sync files"), action='store_true')
		if(debug):
			parser.add_argument('--fullsync', help="Syncs only changed files", action='store_true')
			parser.add_argument('--test', help="For quick testing", action='store_true')
		parser.add_argument('--version', action='version', version='%(prog)s '+self.version)
		parser.add_argument('--shrtLink', metavar=takesfile, type=str, dest='shrtLink', help=_("Make short link for file"))
		args = parser.parse_args()
		#print(args)
		if not args.http_proxy==None and not args.https_proxy==None:
			print(_("Saving proxies"))
			self.setup_proxies(args.http_proxy, args.https_proxy)
		try:
			self.init_settings()		
		except:
			print(_("Need to setup or resetup settings file!"))
			self.setup()
			return
		if not args.ActionDir:
			#should this also update the JSON?
		...
		if args.lsh:			
			...
			fileid = self.uni_get_name(args.dlFileName, "id", "file")
			if not fileid==None:
				#self.download_fileid(fileid, "id", "file"), None)
				self.downloadThreaded(fileid, None)
			else:
				print(_("File not found: ") + args.dlFileName)
			...
			##problem here is that they can be either folders or files
			for item in range(0, self.rootJSON['item_collection']['total_count']):
				#print self.rootJSON['item_collection']['entries'][item]['type']
				if self.rootJSON['item_collection']['entries'][item]['type']=="folder":
					folderID = self.rootJSON['item_collection']['entries'][item]['id']
					#print folderID
					folderName = self.rootJSON['item_collection']['entries'][item]['name']
			...
			self.rm_folder(self.uni_get_name(args.delFolder, "id", "folder"))
		elif not args.delFile==None:
			self.rm_file(self.uni_get_name(args.delFile, "id", "file"))
		elif not args.shrtLink==None:
			googl = googlShort()
			itemID = self.uni_get_name(args.shrtLink, "id", "folder")
			item_type = "folder"
			if(itemID == None):
				itemID = self.uni_get_name(args.shrtLink, "id", "file")
				item_type = "file"
			longURL = self.get_item_url(itemID, item_type)
			print(googl.shorten_url(longURL))
		#elif args.test:
		#	print(self.random_string(10))
		else:
			print(_("Try running `boxlinux --help` or `boxlinux --setup`"))
	"""
	...
	def setup(self):
		...
		#if they are using python 1... well they have more pressing problems
		enter_prompt = _("Press enter when approved")
		if(sys.version_info[0]==2): 
			raw_input(enter_prompt)
		else:
			input(enter_prompt)
